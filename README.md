...
To run the API and PostgreSQL together with Docker Compose:

```bash
docker compose up --build
```

Open <http://127.0.0.1:8000/> for the token-based local dashboard. Set
`RELAY_DATABASE_URL` to use another database URL. `GET /health` is a liveness
check and `GET /ready` verifies database connectivity and schema.
...
`main.py` and `schemas.py`. PostgreSQL row locks using `FOR UPDATE SKIP LOCKED`
make concurrent claims safe across processes without changing the HTTP
protocol or lifecycle in `SPEC.md`.
...
Tests explicitly default to a repository-local SQLite scratch database so they
do not reset a running PostgreSQL instance. The fixture drops and recreates
all tables on whatever `RELAY_DATABASE_URL` points at, so use a scratch
database before running tests against another database.

This project intentionally does not include Kubernetes, CI, external brokers,
or an LLM. PostgreSQL is the default persistence layer; Docker Compose is
